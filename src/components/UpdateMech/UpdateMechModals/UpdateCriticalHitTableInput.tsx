...
interface UpdateCriticalHitTableInputProps {
  number: string;
...
export const UpdateCriticalHitTableInput: React.FC<UpdateCriticalHitTableInputProps> = ({ number, input, value, handleInput, handleChange, bodyPart, handleValue }) => {
  return (
    <div className="w-full flex pt-4">
      ...
      {input[value as keyof CriticalHitTableType] === "input" ? (
        <input
          ...
          name={`${bodyPart}.${value}`}
          placeholder={handleValue(bodyPart)?.[value as keyof CriticalHitTableType]}
          onChange={(e) => {
            handleChange(bodyPart, `${value}`, e);
          }}

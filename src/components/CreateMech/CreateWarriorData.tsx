...
import { CreateWarriorDataModal } from "./CreateMechModals/CreateWarriorDataModal";
...
export const CreateWarriorData: React.FC<CreateWarriorDataProps> = ({ id, mechInfo }) => {
  ...
  const closeModal = (e: React.MouseEvent<HTMLButtonElement>): void => {
    e.preventDefault()
    setModal(false);
  };

  const saveModal = (e: React.MouseEvent<HTMLButtonElement>): void => {
    e.preventDefault()
    saveWarriorData()
    setModal(false);
  };
  ...
  return (
    <div className="grid grid-cols-16 grid-rows-9 w-full h-full">
      ...
      <CreateWarriorDataModal modal={modal} warriorData={warriorData} handleChange={handleChange} closeModal={closeModal} saveModal={saveModal}/>
      ...
      <p className="col-start-1 col-end-7 row-start-6 row-end-7 text-2xs font-bold text-right">Hits </p>
